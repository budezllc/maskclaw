...
pub struct ServerState {
    ...
    #[cfg(feature = "maskclaw")]
    maskclaw: Option<Arc<maskclaw::Engine>>,
}
...
impl ServerState {
    ...
    /// Attaches a MaskClaw engine. `None` leaves requests unscrubbed.
    #[cfg(feature = "maskclaw")]
    pub fn with_maskclaw(mut self, engine: Option<Arc<maskclaw::Engine>>) -> Self {
        self.maskclaw = engine;
        self
    }
...
pub fn build_switchyard_router(state: ServerState) -> Router {
    ...
    #[cfg(feature = "maskclaw")]
    {
        router = router.route("/v1/maskclaw/stats", get(maskclaw_hook::get_stats));
    }
...
async fn anthropic_count_tokens(
    ...
    #[cfg(feature = "maskclaw")]
    let maskclaw_outcome = match maskclaw_hook::scrub_request(&state, &mut request).await {
        Ok(outcome) => outcome,
        Err(response) => return anthropic_error_response(response),
    };
    #[cfg(feature = "maskclaw")]
    let route = maskclaw_hook::select_route(&state, route, maskclaw_outcome.as_ref());
...
async fn handle_llm_request(
    ...
    let cache_probe = {
        #[cfg(feature = "maskclaw")]
        let skip_raw = state.maskclaw.is_some();
        #[cfg(not(feature = "maskclaw"))]
        let skip_raw = false;
        (state.track_cache_eligibility && !skip_raw).then(|| prefix_probe(&body))
    };
    ...
    #[cfg(feature = "maskclaw")]
    let maskclaw_engine = state.maskclaw.clone();
    ...
    #[cfg(feature = "maskclaw")]
    let response = maskclaw_hook::restore_response(
        maskclaw_engine.as_ref(),
        response,
        maskclaw_outcome.as_ref(),
    );
...
fn codex_model_entry_json(model: &str, capabilities: ModelCapabilities, priority: usize) -> Value {
    ...
    json!({
        "slug": model,
        "display_name": model,
        "description": "MaskClaw-routed model.",
...
fn startup_banner(options: &ServerRunOptions, state: &ServerState, color: bool) -> String {
    ...
    let example_model = routes.first().copied().unwrap_or("maskclaw/route");
    let example_body = json!({
        "model": example_model,
        "messages": [{"role": "user", "content": "Hello from MaskClaw"}],
    });
...
// Keep redirected logs plain. Terminal output uses MaskClaw copper, not NVIDIA green.
...
fn endpoint_listing(has_routing_log: bool) -> String {
    ...
    #[cfg(feature = "maskclaw")]
    endpoints.push("  GET  /v1/maskclaw/stats     privacy stats");
    if has_routing_log {
