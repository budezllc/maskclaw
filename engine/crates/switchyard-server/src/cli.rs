...
#[command(
    ...
    about = "MaskClaw privacy proxy (forked from NVIDIA NeMo Switchyard)",
    version
)]
pub(crate) struct ServerArgs {
    ...
    /// Optional MaskClaw sidecar TOML. Defaults to maskclaw.toml next to --config.
    #[cfg(feature = "maskclaw")]
    #[arg(long, value_name = "PATH")]
    maskclaw_config: Option<PathBuf>,
...
impl ServerArgs {
    ...
    fn into_runtime(self) -> ServerResult<(ServerState, ServerRunOptions)> {
        ...
        #[cfg(feature = "maskclaw")]
        {
            let engine = maskclaw::load_sidecar(&self.config, self.maskclaw_config.as_deref())
                .map_err(|error| ServerError::new(error.to_string()))?;
            state = state.with_maskclaw(engine.map(std::sync::Arc::new));
        }
